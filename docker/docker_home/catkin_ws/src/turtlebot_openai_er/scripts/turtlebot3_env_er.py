...
class TurtleBot3Env(robot_gazebo_env.RobotGazeboEnv):
	...
	* /camera
	...
	while self.camera is None and not rospy.is_shutdown():
	    try:
		self.camera = rospy.wait_for_message("/camera/rgb/image_raw", Image, timeout=1.0)
		rospy.logdebug("Current /camera/rgb/image_raw READY=>")

	    except:
		rospy.logerr("Current /camera/rgb/image_raw not ready yet, retrying for getting camera")
	return self.camera
